...
rcd_authority=$(jq -r '.authority' package.json | sed 's/null//')
...
next_ver=$(laconic -c $CONFIG_FILE registry record list --type ApplicationRecord --all --name "$rcd_authority" 2>/dev/null | jq -r -s ".[] | sort_by(.createTime) | reverse | [ .[] | select(.bondId == \"$CERC_REGISTRY_BOND_ID\") ] | .[0].attributes.version" | awk -F. -v OFS=. '{$NF += 1 ; print}')
...
if [ -z "$CERC_REGISTRY_APP_CRN" ]; then
  authority=$(echo "$rcd_authority" | cut -d'/' -f1 | sed 's/@//')
  app=$(echo "$rcd_authority" | cut -d'/' -f2-)
...
record:
  ...
  version: 1.0.0
  name: "$rcd_authority@$rcd_app_version"
  application: "$CERC_REGISTRY_APP_CRN@$rcd_app_version"
  dns: `echo $rcd_homepage | cut -d'.' -f1`
